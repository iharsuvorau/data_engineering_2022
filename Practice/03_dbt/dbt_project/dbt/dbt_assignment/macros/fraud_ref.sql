...
-- Taken from https://www.postgresql.org/message-id/4A091D25.6070108%40postnewspapers.com.au
...
(SELECT
     ...
     MOD(($1::int8 / (10 ^ n)::int8), 10::int8)
         ...
         * (MOD(n, 2) + 1)
         AS doubled_digit
 FROM generate_series(0, ceil(log($1))::integer - 1) AS n) AS doubled_digits;
